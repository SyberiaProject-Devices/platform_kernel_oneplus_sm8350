...
#define OPLUS_CHG_UPDATE_INIT_DELAY                                            \
...
#define OPLUS_CHG_UPDATE_INTERVAL(slow)                                        \
	round_jiffies_relative(msecs_to_jiffies(                               \
		...
		1000))
#else
#define OPLUS_CHG_UPDATE_INTERVAL                                              \
	round_jiffies_relative(                                                \
		msecs_to_jiffies(OPLUS_CHG_UPDATE_INTERVAL_SEC * 1000))
#endif
...
#define charger_xlog_printk(num, fmt, ...)                                     \
	do {                                                                   \
		if (enable_charger_log >= (int)num) {                          \
			printk(KERN_NOTICE pr_fmt("[OPLUS_CHG][%s]" fmt),      \
			  ...
			  union power_supply_propval *val)
{
	...
	if (chip->charger_exist) {
		if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP ||
		    ...
		    (oplus_warp_get_adapter_update_status() ==
					...
					enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (g_charger_chip &&
		    g_charger_chip->smart_charging_screenoff) {
			rc = 1;
		} else {
			rc = 0;
		}
			       ...
			       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SHORT_C_LIMIT_RECHG:
		printk(KERN_ERR
		       "[OPLUS_CHG] [short_c_bat] set limit rechg[%d]\n",
		       !!val->intval);
	...
	case POWER_SUPPLY_PROP_SHORT_C_BATT_UPDATE_CHANGE:
		printk(KERN_ERR
		       "[OPLUS_CHG] [short_c_batt]: set update change[%d]\n",
		       val->intval);
	...
	case POWER_SUPPLY_PROP_SHORT_C_HW_FEATURE:
		printk(KERN_ERR
		       "[OPLUS_CHG] [short_c_hw_check]: set is_feature_hw_on [%d]\n",
		       val->intval);
...
static void oplus_midas_chg_info(const char *name)
{
	midas_debug("%s: passedchg=%d, realpassedchg=%d,"
		    ...
		    midas_chg.accu_delta, midas_chg.prev_chg_stat,
			       ...
			       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		if (chip->warp_show_ui_soc_decimal == true &&
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		val->intval =
			chip->ui_soc * chip->batt_capacity_mah * 1000 / 100;
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		val->intval = 2000;
		break;
	...
	case POWER_SUPPLY_PROP_BATTERY_RM:
		if (oplus_warp_get_fastchg_started() == true) {
			chip->batt_rm =
				oplus_gauge_get_prev_remaining_capacity() *
				chip->vbatt_num;
		} else {
			chip->batt_rm = oplus_gauge_get_remaining_capacity() *
					chip->vbatt_num;
		}
...
static bool oplus_chg_is_wls_online(struct oplus_chg_chip *dev)
{
	...
	rc = oplus_chg_mod_get_property(dev->wls_ocm, OPLUS_CHG_PROP_ONLINE,
					&pval);
	if (rc < 0)
		return false;
...
static bool oplus_chg_is_wls_present(struct oplus_chg_chip *dev)
{
	...
	rc = oplus_chg_mod_get_property(dev->wls_ocm, OPLUS_CHG_PROP_PRESENT,
					&pval);
	if (rc < 0)
		return false;
...
static enum oplus_chg_wls_type
oplus_chg_get_wls_charge_type(struct oplus_chg_chip *dev)
{
	...
	rc = oplus_chg_mod_get_property(dev->wls_ocm, OPLUS_CHG_PROP_WLS_TYPE,
					&val);
	if (rc < 0) {
...
static int oplus_chg_get_skin_temp(struct oplus_chg_chip *dev, int *temp)
{
	...
	rc = oplus_chg_mod_get_property(dev->comm_ocm, OPLUS_CHG_PROP_SKIN_TEMP,
					&pval);
	if (rc < 0)
		return rc;
	*temp = pval.intval;
...
static int init_proc_batt_param_noplug(void)
{
	...
	p = proc_create("batt_param_noplug", 0664, NULL,
			&batt_param_noplug_proc_fops);
	if (!p) {
...
static ssize_t proc_tbatt_pwroff_read(struct file *filp, char __user *buff,
				      size_t count, loff_t *off)
{
	char page[256] = { 0 };
	char read_data[3] = { 0 };
...
static ssize_t chg_log_write(struct file *filp, const char __user *buff,
...
static ssize_t chg_log_read(struct file *filp, char __user *buff, size_t count,
			    loff_t *off)
{
	char page[256] = { 0 };
	char read_data[32] = { 0 };
			       ...
			       size_t count, loff_t *ppos)
{
	...
	if (strncmp(proc_chg_cycle_data, "en808", 5) == 0) {
		...
		charger_xlog_printk(CHG_LOG_CRTI,
				    "powersave: stop usbtemp monitor, etc.\n");
		g_charger_chip->chg_powersave = true;
	} else if (strncmp(proc_chg_cycle_data, "unpowersave", 11) == 0) {
		charger_xlog_printk(
			CHG_LOG_CRTI,
			"unpowersave: start usbtemp monitor, etc.\n");
		g_charger_chip->chg_powersave = false;
	} else {
		return -EFAULT;
	}
...
{
	if (!proc_create("charger_cycle", S_IWUSR | S_IWGRP | S_IWOTH, NULL,
			 &chg_cycle_proc_fops)) {
		chg_err("proc_create chg_cycle_proc_fops fail!\n");
	}
}
static ssize_t critical_log_read(struct file *filp, char __user *buff,
				 size_t count, loff_t *off)
{
	char page[256] = { 0 };
	char read_data[32] = { 0 };
...
static ssize_t critical_log_write(struct file *filp, const char __user *buff,
				  size_t len, loff_t *data)
{
	char write_data[32] = { 0 };
	int critical_log = 0;
...
static ssize_t rtc_reset_read(struct file *filp, char __user *buff,
			      size_t count, loff_t *off)
{
	char page[256] = { 0 };
	char read_data[32] = { 0 };
...
static ssize_t vbat_low_read(struct file *filp, char __user *buff, size_t count,
			     loff_t *off)
{
	char page[256] = { 0 };
	char read_data[32] = { 0 };
						   ...
						   size_t count, loff_t *lo)
{
	...
	if (buffer[0] == '0') {
		...
		chg_err("chip->factory_mode=%d\n", chip->factory_mode);
		chip->limits.vbatt_pdqc_to_9v_thr =
			oplus_get_vbatt_pdqc_to_9v_thr();
		chg_err("vbatt_pdqc_to_9v_thr=%d\n",
			chip->limits.vbatt_pdqc_to_9v_thr);
	}
...
static ssize_t proc_charger_passedchg_read(struct file *filp, char __user *buff,
					   size_t count, loff_t *off)
{
	char page[256] = { 0 };
	char read_data[128] = { 0 };
...
static int init_charger_proc(struct oplus_chg_chip *chip)
{
	...
	prEntry_tmp =
		proc_create_data("charger_factorymode_test", 0666, prEntry_da,
	...
	prEntry_tmp =
		proc_create_data("input_current_now", 0666, prEntry_da,
	...
	prEntry_tmp =
		proc_create_data("passedchg_reset_count", 0666, prEntry_da,
...
static ssize_t proc_ui_soc_decimal_read(struct file *filp, char __user *buff,
					size_t count, loff_t *off)
{
	char page[256] = { 0 };
	char read_data[128] = { 0 };
	...
	if (chip->warp_show_ui_soc_decimal) {
		...
		if (swarp_is_control_by_warp != true &&
			...
			schedule_work = mod_delayed_work(
				system_wq, &chip->ui_soc_decimal_work, 0);
		}

		if (control_by_wireless && chip->decimal_control == true) {
			cancel_delayed_work_sync(
				&g_charger_chip->ui_soc_decimal_work);
			chip->last_decimal_ui_soc =
				(chip->ui_soc_integer + chip->ui_soc_decimal);
			oplus_chg_ui_soc_decimal_deinit();
			chip->calculate_decimal_time = 0;
			pr_err("[proc_ui_soc_decimal_read]cancel last_decimal_ui_soc:%d",
			       chip->last_decimal_ui_soc);
		}
...
void oplus_chg_ui_soc_decimal_init(void)
{
	struct oplus_chg_chip *chip = g_charger_chip;
	if (oplus_warp_get_fastchg_started() == true) {
		chip->batt_rm = oplus_gauge_get_prev_remaining_capacity() *
				chip->vbatt_num;
		chip->batt_fcc =
			oplus_gauge_get_prev_batt_fcc() * chip->vbatt_num;
	} else {
		chip->batt_rm =
			oplus_gauge_get_remaining_capacity() * chip->vbatt_num;
		chip->batt_fcc = oplus_gauge_get_batt_fcc() * chip->vbatt_num;
	}
	pr_err("[oplus_chg_ui_soc_decimal_init]!!!soc:%d",
	       (int)((chip->batt_rm * 10000) / chip->batt_fcc));
	...
	if (chip->ui_soc == 100) {
		...
		chip->ui_soc_integer = chip->ui_soc * 1000;
		chip->ui_soc_decimal =
			chip->batt_rm * 100000 / chip->batt_fcc -
			(chip->batt_rm * 100 / chip->batt_fcc) * 1000;
		if ((chip->ui_soc_integer + chip->ui_soc_decimal) >
			    chip->last_decimal_ui_soc &&
		    chip->last_decimal_ui_soc != 0) {
			chip->ui_soc_decimal =
				((chip->last_decimal_ui_soc % 1000 - 50) > 0) ?
					(chip->last_decimal_ui_soc % 1000 -
					 50) :
					      0;
		}
	}
...
void oplus_chg_ui_soc_decimal_deinit(void)
{
	struct oplus_chg_chip *chip = g_charger_chip;
	chip->ui_soc_integer =
		(chip->ui_soc_integer + chip->ui_soc_decimal) / 1000;
...
static void oplus_chg_show_ui_soc_decimal(struct work_struct *work)
{
	...
	if (icharging > 0) {
		speed = 100000 * icharging * UPDATE_TIME * chip->vbatt_num /
			(chip->batt_fcc * 3600) / ratio;
		pr_err("[oplus_chg_show_ui_soc_decimal] icharging = %d, batt_fcc :%d",
		       chip->icharging, chip->batt_fcc);
	} else {
	...
	pr_err("[oplus_chg_ui_soc_decimal]chip->ui_soc_decimal+chip_ui_soc: %d , speed: %d, soc :%d\n ",
	       (chip->ui_soc_decimal + chip->ui_soc_integer), speed,
	...
	if (chip->calculate_decimal_time <= MAX_UI_DECIMAL_TIME) {
		...
		schedule_delayed_work(&chip->ui_soc_decimal_work,
				      msecs_to_jiffies(UPDATE_TIME * 1000));
	} else {
		oplus_chg_ui_soc_decimal_deinit();
	}
}
...
static void oplus_chg_ctrl_lcm_work(struct work_struct *work)
{
	...
	if (((oplus_chg_get_charger_voltage() > CHG_VUSBIN_VOL_THR)) ||
		...
		mod_delayed_work(
			system_highpri_wq, &chip->ctrl_lcm_frequency,
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chg = container_of(dwork, struct oplus_chg_chip,
						  recovery_chg_type_work);

	if (!chg) {
		dev_err(chg->dev, "chip null\n");
		return;
	}
	if (chg->hw_detected && chg->abnormal_volt_detected &&
	    (chg->charger_volt >= VOLT_HIGH_VBUS_VALUE)) {
...
int oplus_chg_init(struct oplus_chg_chip *chip)
{
	...
	chip->shortc_thread =
		kthread_create(shortc_thread_main, (void *)chip, thread_name);
	...
	INIT_DELAYED_WORK(&chip->check_abnormal_voltage_work,
			  oplus_check_abnormal_voltage_work);
	INIT_DELAYED_WORK(&chip->recovery_chg_type_work,
			  oplus_recovery_chg_type_work);
#endif
	INIT_DELAYED_WORK(&chip->update_work, oplus_chg_update_work);
	INIT_DELAYED_WORK(&chip->ui_soc_decimal_work,
			  oplus_chg_show_ui_soc_decimal);
	INIT_DELAYED_WORK(&chip->reset_adapter_work,
			  oplus_chg_reset_adapter_work);
#ifdef OPLUS_CHG_OP_DEF
#ifndef CONFIG_OPLUS_CHG_OOS
	INIT_DELAYED_WORK(&chip->led_power_on_report_work,
			  oplus_chg_led_power_on_report_work);
#endif
#endif
...
int oplus_chg_parse_swarp_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,warp_project",
				  &chip->warp_project);
	if (rc < 0) {
		chip->warp_project = 0;
	}
	chip->platform_fg_flag =
		of_property_read_bool(node, "qcom,platform_fg_flag");
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	if (rc) {
		chip->limits.input_current_usb_ma =
			OPCHG_INPUT_CURRENT_LIMIT_USB_MA;
	}
	...
	if (get_eng_version() == HIGH_TEMP_AGING) {
		...
		if (rc) {
			chip->limits.input_current_led_ma =
				OPCHG_INPUT_CURRENT_LIMIT_LED_MA;
		}
	}
	...
	if (get_eng_version() == HIGH_TEMP_AGING) {
		...
		rc = of_property_read_u32(
			node, "qcom,input_current_led_ma_high",
			&chip->limits.input_current_led_ma_high);
		if (rc) {
			chip->limits.input_current_led_ma_high =
				chip->limits.input_current_led_ma;
	...
	if (get_eng_version() == HIGH_TEMP_AGING) {
		...
		rc = of_property_read_u32(
			node, "qcom,input_current_led_ma_warm",
			&chip->limits.input_current_led_ma_warm);
		if (rc) {
			chip->limits.input_current_led_ma_warm =
				chip->limits.input_current_led_ma;
	...
	if (get_eng_version() == HIGH_TEMP_AGING) {
		...
		rc = of_property_read_u32(
			node, "qcom,input_current_led_ma_normal",
			&chip->limits.input_current_led_ma_normal);
		if (rc) {
			chip->limits.input_current_led_ma_normal =
				chip->limits.input_current_led_ma;
	...
	if (rc) {
		chip->limits.input_current_camera_ma =
			OPCHG_INPUT_CURRENT_LIMIT_CAMERA_MA;
	}
#ifdef OPLUS_CHG_OP_DEF
	...
	if (rc) {
		chip->limits.batt_current_camera_ma =
			OPCHG_BATT_CURRENT_LIMIT_CAMERA_MA;
	}
	rc = of_property_read_u32(node, "qcom,fast_current_camera_level",
				  &chip->limits.fast_current_camera_level);
	if (rc) {
		chip->limits.fast_current_camera_level =
			OPCHG_FAST_CURRENT_LIMIT_CAMERA_LEVEL;
	}
#endif
	chip->limits.iterm_disabled =
		of_property_read_bool(node, "qcom,iterm_disabled");
	rc = of_property_read_u32(node, "qcom,iterm_ma",
				  &chip->limits.iterm_ma);
	if (rc < 0) {
		chip->limits.iterm_ma = -EINVAL;
	}
	chip->smart_charging_screenoff =
		of_property_read_bool(node, "qcom,smart_charging_screenoff");
	rc = of_property_read_u32(node, "qcom,input_current_calling_ma",
				  &chip->limits.input_current_calling_ma);
	if (rc) {
		chip->limits.input_current_calling_ma =
			OPCHG_INPUT_CURRENT_LIMIT_CALLING_MA;
	}
#ifdef OPLUS_CHG_OP_DEF
	...
	if (rc < 0) {
		...
		chip->limits.removed_bat_decidegc =
			-batt_removed_degree_negative;
	}
	/*-3~0 C*/
	...
	rc = of_property_read_u32(
		node, "qcom,temp_little_cold_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_little_cold_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_little_cold_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_little_cold_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_little_cold_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_cool_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_cool_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_cool_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_little_cool_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_little_cool_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_little_cool_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_little_cool_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_little_cool_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_normal_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_normal_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_phase1_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_phase2_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_phase2_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_phase3_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_phase3_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_phase4_fastchg_current_ma_high",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_phase4_fastchg_current_ma_low",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_phase5_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_normal_phase6_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,pd_temp_warm_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,qc_temp_warm_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,temp_warm_fastchg_current_ma_led_on",
	...
	rc = of_property_read_u32(
		node, "qcom,non_standard_fastchg_current_ma",
	...
	chip->limits.sw_vfloat_over_protect_enable = of_property_read_bool(
		node, "qcom,sw_vfloat_over_protect_enable");
	rc = of_property_read_u32(
		node, "qcom,non_standard_vfloat_over_sw_limit",
	...
	rc = of_property_read_u32(
		node, "qcom,little_cold_vfloat_over_sw_limit",
	...
	rc = of_property_read_u32(
		node, "qcom,little_cool_vfloat_over_sw_limit",
	...
	charger_xlog_printk(CHG_LOG_CRTI, "vbatt_power_off = %d, \
			...
			vbatt_pdqc_to_5v_thr = %d\n",
			    chip->vbatt_power_off, chip->vbatt_soc_1,
			    chip->limits.normal_vterm_hw_inc,
			    chip->limits.non_normal_vterm_hw_inc,
			    chip->limits.vbatt_pdqc_to_9v_thr,
			    chip->limits.vbatt_pdqc_to_5v_thr);
	...
	rc = of_property_read_u32(
		node, "qcom,ffc2_normal_vfloat_over_sw_limit",
	...
	rc = of_property_read_u32(
		node, "qcom,default_temp_normal_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,default_normal_vfloat_over_sw_limit",
	...
	rc = of_property_read_u32(
		node, "qcom,default_temp_little_cool_fastchg_current_ma",
	...
	rc = of_property_read_u32(
		node, "qcom,default_little_cool_vfloat_sw_limit",
	...
	rc = of_property_read_u32(
		node, "qcom,default_temp_little_cool_vfloat_mv",
	...
	rc = of_property_read_u32(
		node, "qcom,default_little_cool_vfloat_over_sw_limit",
	...
	chip->chg_ctrl_by_warp =
		of_property_read_bool(node, "qcom,chg_ctrl_by_warp");
	chip->chg_ctrl_by_warp_default =
		of_property_read_bool(node, "qcom,chg_ctrl_by_warp");
	...
	rc = of_property_read_u32(
		node, "qcom,input_current_warp_led_ma_normal",
	...
	chip->suspend_after_full =
		of_property_read_bool(node, "qcom,suspend_after_full");
	chip->check_batt_full_by_sw =
		of_property_read_bool(node, "qcom,check_batt_full_by_sw");
	chip->external_gauge =
		of_property_read_bool(node, "qcom,external_gauge");
	chip->fg_bcl_poll =
		of_property_read_bool(node, "qcom,fg_bcl_poll_enable");

	chip->wireless_support =
		of_property_read_bool(node, "qcom,wireless_support");
	chip->wpc_no_chargerpump =
		of_property_read_bool(node, "qcom,wpc_no_chargerpump");
	chip->chg_ctrl_by_lcd =
		of_property_read_bool(node, "qcom,chg_ctrl_by_lcd");
	chip->chg_ctrl_by_lcd_default =
		of_property_read_bool(node, "qcom,chg_ctrl_by_lcd");
	chip->chg_ctrl_by_camera =
		of_property_read_bool(node, "qcom,chg_ctrl_by_camera");
	chip->bq25890h_flag = of_property_read_bool(node, "qcom,bq25890_flag");
	chip->chg_ctrl_by_calling =
		of_property_read_bool(node, "qcom,chg_ctrl_by_calling");
	...
	chip->new_ui_warning_support =
		of_property_read_bool(node, "qcom,new_ui_warning_support");
	chip->recharge_after_full =
		of_property_read_bool(node, "recharge_after_full");
	...
	if (get_eng_version() == HIGH_TEMP_AGING ||
		...
		chip->limits.max_chg_time_sec =
			chip->batt_capacity_mah / 250 * 3600;
	}

#ifdef OPLUS_CHG_OP_DEF
	chip->support_abnormal_vol_check =
		of_property_read_bool(node, "qcom,abnormal_volt_check");
	charger_xlog_printk(CHG_LOG_CRTI,
			    "chip->support_abnormal_vol_check:%d\n",
			    chip->support_abnormal_vol_check);
#endif
	charger_xlog_printk(
		CHG_LOG_CRTI, "input_current_charger_ma = %d, \
		...
		chip->limits.input_current_led_ma_high,
		chip->limits.temp_normal_fastchg_current_ma,
		chip->limits.temp_normal_vfloat_mv, chip->limits.iterm_ma,
		chip->limits.recharge_mv, chip->limits.cold_bat_decidegc,
		chip->limits.temp_cold_vfloat_mv,
		chip->limits.temp_cold_fastchg_current_ma,
		chip->limits.little_cold_bat_decidegc,
		chip->limits.temp_little_cold_vfloat_mv,
		...
		chip->warp_project, chip->suspend_after_full,
		chip->external_gauge,
		chip->limits.sw_vfloat_over_protect_enable,
		chip->limits.temp_little_cold_fastchg_current_ma_low,
		chip->limits.temp_little_cold_fastchg_current_ma_high,
		chip->limits.charger_current_warp_ma_normal, chip->ffc_support,
		chip->dual_ffc, chip->new_ui_warning_support,
		chip->smooth_switch);

	chip->dual_charger_support =
		of_property_read_bool(node, "qcom,dual_charger_support");
	...
	rc = of_property_read_u32(node, "qcom,slave_chg_enable_ma",
				  &chip->slave_chg_enable_ma);
	if (rc) {
		chip->slave_chg_enable_ma = 2100;
	}

	rc = of_property_read_u32(node, "qcom,slave_chg_disable_ma",
				  &chip->slave_chg_disable_ma);
	if (rc) {
		chip->slave_chg_disable_ma = 1700;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_batttemp_gap",
				  &chip->usbtemp_batttemp_gap);
	if (rc) {
		chip->usbtemp_batttemp_gap = 17;
	}

	chip->warp_show_ui_soc_decimal =
		of_property_read_bool(node, "qcom,warp_show_ui_soc_decimal");

	rc = of_property_read_u32(node, "qcom,ui_soc_decimal_speedmin",
				  &chip->ui_soc_decimal_speedmin);
	if (rc) {
	...
	rc = read_chg_strategy_data_from_node(
		node, "oplus,warp_chg_led_on_strategy_data",
	...
	for (i = 0; i < rc; i++) {
		chg_err("warp_chg_led_on_strategy[%d]: %d %d %d %d %d\n", i,
			chip->dynamic_config.warp_chg_led_on_strategy_data[i]
				.cool_temp,
			chip->dynamic_config.warp_chg_led_on_strategy_data[i]
				.heat_temp,
			chip->dynamic_config.warp_chg_led_on_strategy_data[i]
	...
	rc = read_chg_strategy_data_from_node(
		node, "oplus,pd9v_chg_led_on_strategy_data",
	...
	for (i = 0; i < rc; i++) {
		chg_err("pd9v_chg_led_on_strategy[%d]: %d %d %d %d %d\n", i,
			chip->dynamic_config.pd9v_chg_led_on_strategy_data[i]
				.cool_temp,
			chip->dynamic_config.pd9v_chg_led_on_strategy_data[i]
				.heat_temp,
			chip->dynamic_config.pd9v_chg_led_on_strategy_data[i]
	...
	rc = read_chg_strategy_data_from_node(
		node, "oplus,pd5v_chg_led_on_strategy_data",
	...
	for (i = 0; i < rc; i++) {
		pr_err("pd5v_chg_led_on_strategy[%d]: %d %d %d %d %d\n", i,
		       chip->dynamic_config.pd5v_chg_led_on_strategy_data[i]
			       .cool_temp,
		       chip->dynamic_config.pd5v_chg_led_on_strategy_data[i]
			       .heat_temp,
		       chip->dynamic_config.pd5v_chg_led_on_strategy_data[i]
	...
	rc = read_chg_strategy_data_from_node(
		node, "oplus,swarp_chg_led_on_strategy_data",
		chip->dynamic_config.swarp_chg_led_on_strategy_data);
	if (rc < 0) {
	...
	for (i = 0; i < rc; i++) {
		pr_err("swarp_chg_led_on_strategy[%d]: %d %d %d %d %d\n", i,
		       chip->dynamic_config.swarp_chg_led_on_strategy_data[i]
			       .cool_temp,
		       chip->dynamic_config.swarp_chg_led_on_strategy_data[i]
			       .heat_temp,
		       chip->dynamic_config.swarp_chg_led_on_strategy_data[i]
	...
	rc = read_chg_strategy_data_from_node(
		node, "oplus,swarp_chg_led_off_strategy_data",
		chip->dynamic_config.swarp_chg_led_off_strategy_data);
	if (rc < 0) {
	...
	for (i = 0; i < rc; i++) {
		pr_err("swarp_chg_led_off_strategy[%d]: %d %d %d %d %d\n", i,
		       chip->dynamic_config.swarp_chg_led_off_strategy_data[i]
			       .cool_temp,
		       chip->dynamic_config.swarp_chg_led_off_strategy_data[i]
			       .heat_temp,
		       chip->dynamic_config.swarp_chg_led_off_strategy_data[i]
	...
	rc = read_chg_strategy_data_from_node(
		node, "oplus,swarp_chg_led_off_strategy_data_high",
		chip->dynamic_config.swarp_chg_led_off_strategy_data_high);
	if (rc < 0) {
		pr_err("read oplus,swarp_chg_led_off_strategy_data_high error, rc=%d\n",
		       rc);
		for (i = 0; i < CHG_STRATEGY_DATA_TABLE_MAX; i++) {
			chip->dynamic_config
				.swarp_chg_led_off_strategy_data_high[i]
				.cool_temp =
				chip->dynamic_config
					.swarp_chg_led_off_strategy_data[i]
					.cool_temp;
			chip->dynamic_config
				.swarp_chg_led_off_strategy_data_high[i]
				.heat_temp =
	...
	rc = read_chg_strategy_data_from_node(
		node, "oplus,normal_chg_led_off_strategy_data",
	...
	for (i = 0; i < rc; i++) {
		pr_err("normal_chg_led_off_strategy[%d]: %d %d %d %d %d\n", i,
		       chip->dynamic_config.normal_chg_led_off_strategy_data[i]
			       .cool_temp,
		       chip->dynamic_config.normal_chg_led_off_strategy_data[i]
			       .heat_temp,
		       chip->dynamic_config.normal_chg_led_off_strategy_data[i]
	...
	rc = read_chg_strategy_data_from_node(
		node, "oplus,swarp_general_chg_strategy_data_low",
		chip->dynamic_config.swarp_general_chg_strategy_data_low);
	if (rc < 0) {
	...
	for (i = 0; i < rc; i++) {
		pr_err("swarp_general_chg_strategy_low[%d]: %d %d %d %d %d\n",
		       i,
		       chip->dynamic_config
			       .swarp_general_chg_strategy_data_low[i]
			       .cool_temp,
		       chip->dynamic_config
			       .swarp_general_chg_strategy_data_low[i]
			       .heat_temp,
	...
	rc = read_chg_strategy_data_from_node(
		node, "oplus,swarp_general_chg_strategy_data_high",
		chip->dynamic_config.swarp_general_chg_strategy_data_high);
	if (rc < 0) {
	...
	for (i = 0; i < rc; i++) {
		pr_err("swarp_general_chg_strategy_high[%d]: %d %d %d %d %d\n",
		       i,
		       chip->dynamic_config
			       .swarp_general_chg_strategy_data_high[i]
			       .cool_temp,
		       chip->dynamic_config
			       .swarp_general_chg_strategy_data_high[i]
			       .heat_temp,
	...
	charger_xlog_printk(
		...
		chip->dual_charger_support, chip->slave_pct,
		chip->slave_chg_enable_ma, chip->slave_chg_disable_ma);
...
int oplus_chg_get_tbatt_normal_charging_current(struct oplus_chg_chip *chip)
{
	...
	case BATTERY_STATUS__NORMAL_PHASE2:
		if (vbatt_higherthan_4180mv) {
			charging_current =
				chip->limits
					.temp_normal_phase2_fastchg_current_ma_low;
		} else {
			charging_current =
				chip->limits
					.temp_normal_phase2_fastchg_current_ma_high;
		}
		break;
	case BATTERY_STATUS__NORMAL_PHASE3:
		if (vbatt_higherthan_4180mv) {
			charging_current =
				chip->limits
					.temp_normal_phase3_fastchg_current_ma_low;
		} else {
			charging_current =
				chip->limits
					.temp_normal_phase3_fastchg_current_ma_high;
		}
		break;
	case BATTERY_STATUS__NORMAL_PHASE4:
		if (vbatt_higherthan_4180mv) {
			charging_current =
				chip->limits
					.temp_normal_phase4_fastchg_current_ma_low;
		} else {
			charging_current =
				chip->limits
					.temp_normal_phase4_fastchg_current_ma_high;
		}
		break;
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
	...
	case BATTERY_STATUS__LITTLE_COLD_TEMP:
		...
		if (vbatt_higherthan_4180mv) {
			charging_current =
				chip->limits
					.temp_little_cold_fastchg_current_ma_low;
		} else {
			charging_current =
				chip->limits
					.temp_little_cold_fastchg_current_ma_high;
		}
		charger_xlog_printk(
			CHG_LOG_CRTI,
	...
	case BATTERY_STATUS__LITTLE_COOL_TEMP:
		if (chip->dual_charger_support) {
			if (vbatt_higherthan_4180mv) {
				charging_current =
					chip->limits
						.temp_little_cool_fastchg_current_ma_low;
			} else {
				charging_current =
					chip->limits
						.temp_little_cool_fastchg_current_ma_high;
			}
			charger_xlog_printk(
				CHG_LOG_CRTI,
				"vbatt_higherthan_4180mv [%d], charging_current[%d]\n",
	...
	case BATTERY_STATUS__NORMAL:
		if (chip->dual_charger_support) {
			charging_current =
				oplus_chg_get_tbatt_normal_charging_current(
					chip);
		} else
			charging_current =
				chip->limits.temp_normal_fastchg_current_ma;
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)) {
		if (!chip->dual_charger_support ||
		    (chip->dual_charger_support && chip->charger_volt > 7500)) {
			if (chip->led_temp_status == LED_TEMP_STATUS__HIGH) {
				if (current_limit >
				    chip->limits.input_current_led_ma_high) {
					current_limit =
						chip->limits
							.input_current_led_ma_high;
				}
			} else if (chip->led_temp_status ==
				   LED_TEMP_STATUS__WARM) {
				if (current_limit >
				    chip->limits.input_current_led_ma_warm) {
					current_limit =
						chip->limits
							.input_current_led_ma_warm;
				}
			} else {
				if (current_limit >
				    chip->limits.input_current_led_ma_normal) {
			...
			charger_xlog_printk(
				CHG_LOG_CRTI,
				"[BATTERY]LED STATUS CHANGED, IS ON\n");
		}
		if ((chip->chg_ctrl_by_camera) && (chip->camera_on) &&
			...
			charger_xlog_printk(
				CHG_LOG_CRTI,
				"[BATTERY]CAMERA STATUS CHANGED, IS ON\n");
		}
	...
	if ((chip->chg_ctrl_by_calling) && (chip->calling_on) &&
		...
		current_limit = chip->limits.input_current_calling_ma;
		charger_xlog_printk(CHG_LOG_CRTI,
				    "[BATTERY]calling STATUS CHANGED, IS ON\n");
	}
	if (chip->chg_ctrl_by_warp && chip->vbatt_num == 2 &&
		...
		if (chip->led_on) {
			if (chip->warp_temp_status == WARP_TEMP_STATUS__HIGH) {
				current_limit =
					chip->limits
						.input_current_warp_led_ma_high;
			} else if (chip->warp_temp_status ==
				   WARP_TEMP_STATUS__WARM) {
				current_limit =
					chip->limits
						.input_current_warp_led_ma_warm;
			} else if (chip->warp_temp_status ==
				   WARP_TEMP_STATUS__NORMAL) {
				current_limit =
					chip->limits
						.input_current_warp_led_ma_normal;
			}
		...
		chg_err("chg_ctrl_by_warp,  \
			...
			chip->led_on, chip->calling_on, current_limit,
			chip->warp_temp_status);
		if (chip->chg_ops->input_current_ctrl_by_warp_write) {
			chip->chg_ops->input_current_ctrl_by_warp_write(
				current_limit);
			return;
		}
	}
	if (chip->chg_ctrl_by_cool_down &&
	...
	charger_xlog_printk(CHG_LOG_CRTI, "led_on = %d, \
		...
		current_limit = %d, \
		led_temp_status = %d\n",
			    chip->led_on, chip->chg_ctrl_by_cool_down,
			    current_limit, chip->led_temp_status);
#endif
...
static void
oplus_chg_check_vbatt_higher_than_4180mv(struct oplus_chg_chip *chip)
{
...
static void oplus_chg_check_ffc_temp_status(struct oplus_chg_chip *chip)
{
	...
	if (batt_temp >= chip->limits.ffc2_temp_high_decidegc) { /*>=40C*/
		...
		if (high_counts >= TEMP_FFC_COUNTS) {
		   ...
		   chip->limits.ffc2_temp_warm_decidegc) { /*>=35C && < 40*/
		...
		if (warm_counts >= TEMP_FFC_COUNTS) {
		   ...
		   chip->limits.ffc2_temp_low_decidegc) { /*>=16C&&<35C*/
					  ...
					  OPLUS_CHG_STOP_VOTER voter)
{
	...
	case CHG_STOP_VOTER__FULL:
		...
		if (oplus_warp_get_allow_reading() == true) {
			if (oplus_chg_check_suspend_or_disable(chip) ==
			    CHG_SUSPEND) {
				chip->chg_ops->charger_suspend();
			} else {
				oplus_chg_turn_off_charging(chip);
...
static void battery_temp_anti_shake_handle(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__HIGH_TEMP) { /*>53C*/
		chip->limits.cold_bat_decidegc =
			chip->anti_shake_bound.cold_bound; /*-3C*/
		chip->limits.little_cold_bat_decidegc =
			chip->anti_shake_bound.little_cold_bound; /*0C*/
		chip->limits.cool_bat_decidegc =
			chip->anti_shake_bound.cool_bound; /*5C*/
		chip->limits.little_cool_bat_decidegc =
			chip->anti_shake_bound.little_cool_bound; /*12C*/
		chip->limits.normal_bat_decidegc =
			chip->anti_shake_bound.normal_bound; /*16C*/
		chip->limits.warm_bat_decidegc =
			chip->anti_shake_bound.warm_bound; /*45C*/
		chip->limits.hot_bat_decidegc =
			chip->anti_shake_bound.hot_bound + low_shake; /*53C*/
	} else if (chip->tbatt_status == BATTERY_STATUS__LOW_TEMP) { /*<-3C*/
		chip->limits.cold_bat_decidegc =
			chip->anti_shake_bound.cold_bound + high_shake;
		chip->limits.little_cold_bat_decidegc =
			chip->anti_shake_bound.little_cold_bound;
		chip->limits.cool_bat_decidegc =
			chip->anti_shake_bound.cool_bound;
		chip->limits.little_cool_bat_decidegc =
			chip->anti_shake_bound.little_cool_bound;
		chip->limits.normal_bat_decidegc =
			chip->anti_shake_bound.normal_bound;
		chip->limits.warm_bat_decidegc =
			chip->anti_shake_bound.warm_bound;
		chip->limits.hot_bat_decidegc =
			chip->anti_shake_bound.hot_bound;
	} else if (chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) { /*-3C~0C*/
		chip->limits.cold_bat_decidegc =
			chip->anti_shake_bound.cold_bound + low_shake;
		chip->limits.little_cold_bat_decidegc =
			chip->anti_shake_bound.little_cold_bound +
			high_shake_0c;
		chip->limits.cool_bat_decidegc =
		...
		chip->limits.hot_bat_decidegc =
			chip->anti_shake_bound.hot_bound;
	} else if (chip->tbatt_status ==
		   BATTERY_STATUS__LITTLE_COLD_TEMP) { /*0C-5C*/
		chip->limits.cold_bat_decidegc =
			chip->anti_shake_bound.cold_bound;
		chip->limits.little_cold_bat_decidegc =
			chip->anti_shake_bound.little_cold_bound + low_shake_0c;
		chip->limits.cool_bat_decidegc =
			chip->anti_shake_bound.cool_bound + high_shake;
		chip->limits.little_cool_bat_decidegc =
			chip->anti_shake_bound.little_cool_bound;
		chip->limits.normal_bat_decidegc =
			chip->anti_shake_bound.normal_bound;
		...
		chip->limits.little_cold_bat_decidegc =
			chip->anti_shake_bound.little_cold_bound;
		chip->limits.cool_bat_decidegc =
			chip->anti_shake_bound.cool_bound + low_shake;
		chip->limits.little_cool_bat_decidegc =
		...
		chip->limits.normal_bat_decidegc =
			chip->anti_shake_bound.normal_bound + high_shake;
		chip->limits.warm_bat_decidegc =
			chip->anti_shake_bound.warm_bound;
		chip->limits.hot_bat_decidegc =
			chip->anti_shake_bound.hot_bound;
		...
		chip->limits.hot_bat_decidegc =
		   ...
		   BATTERY_STATUS__WARM_TEMP) { /*45C~53C*/
		chip->limits.cold_bat_decidegc =
			chip->anti_shake_bound.cold_bound;
		chip->limits.little_cold_bat_decidegc =
			chip->anti_shake_bound.little_cold_bound;
		...
		chip->limits.normal_bat_decidegc =
			chip->anti_shake_bound.normal_bound;
		chip->limits.warm_bat_decidegc =
			chip->anti_shake_bound.warm_bound + low_shake;
		chip->limits.hot_bat_decidegc =
			chip->anti_shake_bound.hot_bound + high_shake;
	...
	if (batt_temp > chip->limits.hot_bat_decidegc ||
		...
		if (batt_temp >= chip->limits.warm_bat_decidegc) { /*45C*/
			tbatt_status = BATTERY_STATUS__WARM_TEMP;
			   ...
			   chip->limits.normal_bat_decidegc) { /*16C*/
			tbatt_status = BATTERY_STATUS__NORMAL;
		} else if (batt_temp >=
			   chip->limits.little_cool_bat_decidegc) { /*12C*/
			...
			tbatt_status = BATTERY_STATUS__COOL_TEMP;
			   ...
			   chip->limits.little_cold_bat_decidegc) { /*0C*/
			tbatt_status = BATTERY_STATUS__LITTLE_COLD_TEMP;
		} else if (batt_temp >=
			   chip->limits.cold_bat_decidegc) { /*-3C*/
	...
	if (tbatt_status != chip->tbatt_status) {
		if (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP ||
			...
			if (chip->soc != 100 && chip->batt_full == true &&
				...
				oplus_chg_voter_charging_start(
					chip, CHG_STOP_VOTER__FULL);
			}
		}
...
{
	int tbatt_normal_cur_shake = chip->temperature, low_shake = 0,
	    high_shake = 0;

	if (tbatt_normal_cur_shake >
	    chip->tbatt_normal_pre_shake) { /*get warmer*/
		low_shake = -HYSTERISIS_DECIDEGC;
		high_shake = 0;
	} else if (tbatt_normal_cur_shake <
		   chip->tbatt_normal_pre_shake) { /*get cooler*/
		low_shake = 0;
		high_shake = HYSTERISIS_DECIDEGC;
	}
...
{
	int batt_temp = chip->temperature;
	OPLUS_CHG_TBATT_NORMAL_STATUS tbatt_normal_status =
		chip->tbatt_normal_status;
	...
	if (chip->tbatt_normal_status != tbatt_normal_status) {
		...
		chg_debug(
			"tbatt_normal_status status change, [%d %d %d %d %d %d %d]\n",
			chip->tbatt_normal_status,
			chip->limits.normal_phase1_bat_decidegc,
			chip->limits.normal_phase2_bat_decidegc,
			chip->limits.normal_phase3_bat_decidegc,
	...
	if (event == MSM_DRM_EARLY_EVENT_BLANK) {
		...
		if (blank == MSM_DRM_BLANK_UNBLANK) {
...
#else
			cancel_delayed_work_sync(
				&g_charger_chip->led_power_on_report_work);
			...
			if (is_usb_ocm_available(g_charger_chip))
				oplus_chg_anon_mod_event(
					g_charger_chip->usb_ocm,
					OPLUS_CHG_EVENT_LCD_OFF);
			if (is_wls_ocm_available(g_charger_chip))
				oplus_chg_anon_mod_event(
					g_charger_chip->wls_ocm,
					OPLUS_CHG_EVENT_LCD_OFF);
#endif
		} else {
...
static void oplus_chg_check_tled_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->temperature >
	    chip->limits.led_high_bat_decidegc_antishake) { /* >37C */
		high_counts++;
		if (high_counts >= TLED_CHANGE_COUNTS) {
		   ...
		   chip->limits
			   .led_warm_bat_decidegc_antishake) { /* >35C && <= 37 */
		warm_counts++;
...
static void oplus_chg_check_warp_temp_status(struct oplus_chg_chip *chip)
{
	...
	if (batt_temp > chip->limits.warp_high_bat_decidegc) { /*>45C*/
		if (oplus_warp_get_fastchg_started() == true) {
		   ...
		   chip->limits
			   .warp_warm_bat_decidegc_antishake) { /*>38C && <= 45*/
		high_counts++;
		if (high_counts >= TWARP_COUNTS) {
		   ...
		   chip->limits
			   .warp_normal_bat_decidegc_antishake) { /*>34C && <= 38*/
		warm_counts++;
...
static void oplus_chg_check_warp_ichging(struct oplus_chg_chip *chip)
{
	if (chip->vbatt_num == 2 && chip->warp_temp_change) {
		...
		if (oplus_warp_get_fast_chg_type() ==
		    CHARGER_SUBTYPE_FASTCHG_WARP) {
			oplus_chg_set_input_current_limit(chip);
		}
	}
...
void oplus_chg_variables_reset(struct oplus_chg_chip *chip, bool in)
{
	...
	chip->short_c_batt.in_idle = true;
...
static void oplus_chg_check_rechg_status(struct oplus_chg_chip *chip)
{
	...
	if (rechging_cnt > D_RECHGING_CNT) {
		...
		oplus_chg_voter_charging_start(
			chip, CHG_STOP_VOTER__FULL); /*now rechging!*/
	}
}
...
void oplus_charger_detect_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->check_chrdet_status()) {
		...
		if (chip->wireless_support &&
		    oplus_wpc_get_fw_updating() == true) {
			return;
		}
#endif
#endif /* CONFIG_OPLUS_CHARGER_MTK */
		...
		if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN ||
		    chip->chg_config_init || chip->chg_redetect_charger_type) {
			chip->chg_config_init = false;
			chip->pd_chging = false;
			chip->chg_redetect_charger_type = false;
#else
		if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN) {
			...
			if (chip->chg_ops->get_real_charger_type) {
				chip->real_charger_type =
					chip->chg_ops->get_real_charger_type();
			}
#endif
			charger_xlog_printk(CHG_LOG_CRTI,
					    "Charger in 1 charger_type=%d\n",
					    chip->charger_type);
			if (oplus_warp_get_fastchg_to_normal() == true ||
			    oplus_warp_get_fastchg_to_warm() == true) {
				charger_xlog_printk(
				...
				charger_resumed =
					chip->chg_ops->check_charger_resume();
				oplus_chg_turn_on_charging(chip);
			}
#else /* CONFIG_OPLUS_CHARGER_MTK */
			else {
...
#ifdef OPLUS_CHG_OP_DEF
			if (is_usb_ocm_available(chip)) {
				oplus_chg_mod_event(chip->usb_ocm,
						    chip->usb_ocm,
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if (oplus_warp_get_fastchg_started() == true) {
		...
		chip->batt_volt_max =
			oplus_gauge_get_prev_batt_mvolts_2cell_max();
		chip->batt_volt_min =
			oplus_gauge_get_prev_batt_mvolts_2cell_min();
		chip->icharging = oplus_gauge_get_prev_batt_current();
	...
	if ((chip->chg_ops->get_charger_current &&
	     oplus_warp_get_allow_reading() == true) &&
	    (chip->chg_ops->get_charging_enable &&
	     (chip->chg_ops->get_charging_enable() == true))) {
	...
	if (ui_soc_cp_flag == 0) {
		if ((chip->soc < 0 || chip->soc > 100 ||
		     (chip->chg_ops->get_rtc_soc() == -ENODEV)) &&
		    retry_counts < RETRY_COUNTS) {
			charger_xlog_printk(
				CHG_LOG_CRTI,
		...
		if ((chip->soc < 0 || chip->soc > 100) && soc_load > 0 &&
		    soc_load <= 100) {
			chip->soc = soc_load;
		}
		if ((soc_load != 0) && ((abs(soc_load - chip->soc)) <= 20)) {
			...
			} else if (chip->suspend_after_full &&
				   !chip->external_gauge) {
			...
			if (!chip->external_gauge && soc_load == 0 &&
			    chip->soc < 5) {
				chip->ui_soc = 0;
			}
		}
		chg_err("[soc ui_soc soc_load smooth_soc smooth_switch] = [%d %d %d %d %d]\n",
			chip->soc, chip->ui_soc, chip->soc_load,
			chip->smooth_soc, chip->smooth_switch);
	}
next:
	return;
}
...
static void oplus_chg_check_aicl_input_limit(struct oplus_chg_chip *chip)
{
	...
	if (aicl_delay_count > AICL_DELAY_15MIN) {
		...
		if (oplus_warp_get_allow_reading() == true) {
		   ...
		   chip->charger_volt > 4450 && chip->charger_volt < 5800) {
		aicl_delay_count = 0;
		if (oplus_warp_get_allow_reading() == true) {
...
static void oplus_chg_check_chg_strategy_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->led_on) {
		...
		} else if (chip->chg_ops->oplus_chg_get_pd_type &&
		...
		if (chip->vbatt_num == 2 &&
			...
			if (chip->charger_volt > 7500)
				...
				chip->strategy = &normal_chg_led_off_strategy;
		} else if (chip->chg_ops->get_charger_subtype() ==
			   CHARGER_SUBTYPE_QC) {
			chip->strategy = &normal_chg_led_off_strategy;
		} else {
			chip->strategy = NULL;
...
void oplus_check_ovp_status(struct oplus_chg_chip *chg)
{
	...
	if (chg->hw_detected && ((chg->charger_volt <= VOLT_LOW_VBUS_VALUE) &&
				 (chg->charger_volt >= VOLT_HIGH_VBUS_VALUE))) {
	...
	if ((volt_diff > VOLT_LOW_DIFF_VALUE) &&
	    (volt_diff < VOLT_HIGH_DIFF_VALUE) &&
	    ((chg->charger_volt >= VOLT_LOW_VBUS_VALUE) &&
	     (chg->charger_volt <= VOLT_HIGH_VBUS_VALUE)) &&
...
static void oplus_check_battery_vol_diff(struct oplus_chg_chip *chg)
{
	...
	rc = oplus_chg_mod_get_property(
		chg->batt_ocm, OPLUS_CHG_PROP_VOLTAGE_NOW_CELL1, &pval);
	if (rc) {
	...
	rc = oplus_chg_mod_get_property(
		chg->batt_ocm, OPLUS_CHG_PROP_VOLTAGE_NOW_CELL2, &pval);
	if (rc) {
	...
	if (abs(vbat_cell_max - vbat_cell_min) > ALLOW_DIFF_VALUE) {
		...
		if (chg->check_battery_vol_count > 5) {
			...
			oplus_chg_voter_charging_stop(
				chg, CHG_STOP_VOTER_BAD_VOL_DIFF);
...
static void oplus_chg_protection_check(struct oplus_chg_chip *chip)
{
	if (false == oplus_chg_check_tbatt_is_good(chip)) {
		...
		oplus_chg_voter_charging_stop(
			chip, CHG_STOP_VOTER__BATTTEMP_ABNORMAL);
	} else {
	...
	if (false == oplus_chg_check_vchg_is_good(chip)) {
		...
		oplus_chg_voter_charging_stop(chip,
		...
		if ((chip->stop_voter & CHG_STOP_VOTER__VCHG_ABNORMAL) ==
			...
			oplus_chg_voter_charging_start(
				chip, CHG_STOP_VOTER__VCHG_ABNORMAL);
		}
	}
...
static void battery_notify_authenticate_check(struct oplus_chg_chip *chip)
{
	if (!chip->authenticate) {
		chip->notify_code |= 1 << NOTIFY_BAT_NOT_CONNECT;
		charger_xlog_printk(CHG_LOG_CRTI,
				    "[BATTERY] bat_authenticate is false!\n");
	}
}
	...
	if (true == chip->vbatt_over) {
		...
		if ((chip->batt_full) && (chip->charger_exist)) {
			if (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP &&
			    chip->ui_soc != 100) {
				chip->notify_code |=
					1 << NOTIFY_BAT_FULL_PRE_HIGH_TEMP;
			} else if ((chip->tbatt_status ==
				    BATTERY_STATUS__COLD_TEMP) &&
				   (chip->ui_soc != 100)) {
				chip->notify_code |=
					1 << NOTIFY_BAT_FULL_PRE_LOW_TEMP;
			} else if (!chip->authenticate) {
				chip->notify_code |= 1
						     << NOTIFY_BAT_NOT_CONNECT;
			} else if (!chip->hmac) {
				chip->notify_code |=
					1 << NOTIFY_BAT_FULL_THIRD_BATTERY;
			} else {
				if (chip->ui_soc == 100) {
					chip->notify_code |= 1
							     << NOTIFY_BAT_FULL;
...
static void oplus_chg_smooth_to_soc(struct oplus_chg_chip *chip)
{
	...
	if (status == DISCHARGING_STATUS) {
		if (chip->soc >= 96 && chip->soc <= 100 &&
			...
			if (chip->smooth_soc > chip->soc) {
				if (capacity == -1) {
					...
					soc_pre = chip->soc;
					smooth_diff =
						capacity / chip->smooth_soc;
				}
				if ((capacity - chip->batt_rm) >= smooth_diff ||
				    ((chip->smooth_soc - chip->soc) >
				     chip->smooth_to_soc_gap)) {
					chip->smooth_soc--;
					capacity = chip->batt_rm;
					smooth_diff =
						capacity / chip->smooth_soc;
		...
		if (chip->smooth_soc > chip->soc) {
			if (soc_pre < chip->soc &&
			    (chip->batt_rm - capacity) >= smooth_diff &&
			    chip->smooth_soc < 100) {
				chip->smooth_soc++;
				capacity = chip->batt_rm;
				soc_pre = chip->soc;
				smooth_diff = capacity / (2 * chip->soc -
							  chip->smooth_soc);
			}
			if (chip->soc < soc_pre) {
				chip->smooth_soc--;
...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
	...
	if (chip->batt_exist && (chip->batt_volt_min < chip->vbatt_power_off) &&
...
#endif
			chg_debug(
				"full [soc ui_soc smooth_soc up_limit] = [%d %d %d %d]\n",
...
#ifndef OPLUS_CHG_OP_DEF
#ifndef WPC_NEW_INTERFACE
	} else if ((chip->charger_exist || oplus_wireless_charge_start()) &&
		   chip->batt_exist &&
		   (CHARGING_STATUS_FAIL != chip->charging_state) &&
		   chip->mmi_chg &&
		   (chip->stop_chg == 1 || chip->charger_type == 5)) {
#else
	} else if ((chip->charger_exist || oplus_wpc_get_status()) &&
		   chip->batt_exist &&
		   (CHARGING_STATUS_FAIL != chip->charging_state) &&
		   chip->mmi_chg &&
		   (chip->stop_chg == 1 || chip->charger_type == 5)) {
#endif
#else /* OPLUS_CHG_OP_DEF */
	} else if ((chip->charger_exist || oplus_chg_is_wls_present(chip)) &&
		   chip->batt_exist &&
		   (CHARGING_STATUS_FAIL != chip->charging_state) &&
		   chip->mmi_chg &&
		   (chip->stop_chg == 1 || chip->charger_type == 5)) {
		...
		if (chip->smooth_soc == chip->ui_soc) {
			...
			if (soc_down_count >= soc_down_limit) {
				...
				if (oplus_chg_show_warp_logo_ornot() == false ||
				    (chip->warp_show_ui_soc_decimal == false ||
				     !chip->decimal_control)) {
					chip->ui_soc--;
				}
			}
...
#endif
			chg_debug(
				"full [soc ui_soc smooth_soc down_limit up_limit] = [%d %d %d %d %d]\n",
				chip->soc, chip->ui_soc, chip->smooth_soc,
				soc_down_limit, soc_up_limit);
		}
		charger_xlog_printk(
			CHG_LOG_CRTI,
			"ui_soc:%d,waiting_for_ffc:%d,fastchg_to_ffc:%d,fastchg_start:%d,chg_type=0x%x\n",
			chip->ui_soc, chip->waiting_for_ffc == false,
			chip->fastchg_to_ffc == false,
			oplus_warp_get_fastchg_started(),
			oplus_warp_get_fast_chg_type());
		if (chip->ui_soc == 100 && chip->fastchg_to_ffc == false &&
		    (oplus_warp_get_fastchg_started() == false ||
		     oplus_warp_get_fast_chg_type() ==
			     CHARGER_SUBTYPE_FASTCHG_WARP)) {
			if (++cnt >= 12) {
...
static void oplus_chg_get_chargerid_voltage(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->check_pdphy_ready &&
	    chip->chg_ops->check_pdphy_ready() == false) {
...
#endif /* CONFIG_OPLUS_CHARGER_MTK */
			chip->chargerid_volt =
				chip->chg_ops->get_chargerid_volt();
...
static void oplus_chg_fast_switch_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->allow_swtich_to_fastchg == false) {
		charger_xlog_printk(CHG_LOG_CRTI,
				    " allow_swtich_to_fastchg == 0,return\n");
		return;
	}
	if ((!chip->authenticate) || (!chip->hmac)) {
		charger_xlog_printk(CHG_LOG_CRTI,
				    "non authenticate or hmac,switch return\n");
		return;
	}
	if (chip->notify_flag == NOTIFY_BAT_OVER_VOL) {
		charger_xlog_printk(CHG_LOG_CRTI,
				    " battery over voltage,return\n");
		return;
	}
#ifndef CONFIG_OPLUS_CHARGER_MTK
#ifndef OPLUS_CHG_OP_DEF
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		...
		if (!oplus_warp_get_fastchg_started() &&
			...
			if (reset_mcu_delay == RESET_MCU_DELAY_15S) {
				...
				if (chip->charger_volt <= 7500) {
					oplus_warp_reset_fastchg_after_usbout();
					chip->chg_ops->set_chargerid_switch_val(
						0);
...
static int oplus_chg_check_sw_full(struct oplus_chg_chip *chip)
{
	...
	if (chip->batt_volt > vbatt_full_vol_sw) {
		if (chip->icharging < 0 &&
		    (chip->icharging * -1) <= chip->limits.iterm_ma) {
			chip->sw_full_count++;
...
static bool oplus_chg_check_ffc_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->fastchg_to_ffc == true) {
		if (chip->fastchg_ffc_status == 1) {
			charger_xlog_printk(
				CHG_LOG_CRTI,
				"icharging:%d,batt_volt:%d,temperature:%d\n",
				chip->icharging, chip->batt_volt,
				chip->temperature);
			if (chip->batt_volt >=
			    chip->limits.ffc1_normal_vfloat_sw_limit) {
				vffc1_counts++;
				if (vffc1_counts >= FFC_VOLT_COUNTS) {
					...
					charger_xlog_printk(
						CHG_LOG_CRTI,
						"batt_volt:%d, ffc1_normal_vfloat_sw_limit:%d, vffc1_counts:%d\n",
						chip->batt_volt,
						chip->limits
							.ffc1_normal_vfloat_sw_limit,
						vffc1_counts);
					return false;
				}
			}
			if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL) {
				if ((chip->icharging * -1) <
					...
					charger_xlog_printk(
						CHG_LOG_CRTI,
						"icharging:%d,ff1_normal_fastchg_ma:%d,ff1_exit_step_ma:%d,normal_counts:%d\n",
						chip->icharging,
						chip->limits
							.ff1_normal_fastchg_ma,
						chip->limits.ff1_exit_step_ma,
						normal_counts);
					if (normal_counts >=
					    FFC_CURRENT_COUNTS) {
						oplus_chg_ffc_variable_reset(
							chip);
						oplus_chg_turn_on_charging(
							chip);
						charger_xlog_printk(
							CHG_LOG_CRTI,
							"ffc normal fail icharging:%d,ff1_normal_fastchg_ma:%d,ff1_exit_step_ma:%d,normal_counts:%d\n",
							chip->icharging,
							chip->limits
								.ff1_normal_fastchg_ma,
							chip->limits
			...
			} else if (chip->ffc_temp_status ==
				   FFC_TEMP_STATUS__WARM) {
				if ((chip->icharging * -1) <
				    (chip->limits.ff1_warm_fastchg_ma -
				     chip->limits.ff1_warm_exit_step_ma)) {
					warm_counts++;
					if (warm_counts >= FFC_CURRENT_COUNTS) {
						...
						charger_xlog_printk(
							CHG_LOG_CRTI,
							"icharging:%d,ff1_warm_exit_step_ma:%d,ff1_warm_fastchg_ma:%d,warm_counts:%d\n",
							chip->icharging,
							chip->limits
								.ff1_warm_exit_step_ma,
							chip->limits
								.ff1_warm_fastchg_ma,
							warm_counts);
						return true;
		...
		if (chip->fastchg_ffc_status == 2) {
			...
			if (chip->batt_volt >= ffc_vfloat_sw_limit) {
				...
				if (vffc2_counts >= FFC_VOLT_COUNTS) {
					...
					charger_xlog_printk(
						CHG_LOG_CRTI,
						"batt_volt:%d,ffc_temp_status:%d,ffc_vfloat_sw_limit:%d,vffc2_counts:%d\n",
						chip->batt_volt,
						chip->ffc_temp_status,
						ffc_vfloat_sw_limit,
						vffc2_counts);
					return true;
				}
			...
			if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL) {
				if ((chip->icharging * -1) <
					...
					if (normal_counts >=
						...
						charger_xlog_printk(
							CHG_LOG_CRTI,
							"icharging:%d,ffc2_normal_fastchg_ma:%d,ffc2_exit_step_ma:%d,normal_counts:%d\n",
							chip->icharging,
							chip->limits
								.ffc2_normal_fastchg_ma,
							chip->limits
								.ffc2_exit_step_ma,
							normal_counts);
						return true;
				...
				if ((chip->icharging * -1) <
					...
					if (warm_counts >= FFC_CURRENT_COUNTS) {
						...
						charger_xlog_printk(
							CHG_LOG_CRTI,
							"icharging:%d:%d,ffc2_warm_fastchg_ma:%d,ffc2_warm_exit_step_ma:%d,warm_counts:%d\n",
							chip->icharging,
							chip->limits
								.ffc2_warm_fastchg_ma,
							chip->limits
								.ffc2_warm_exit_step_ma,
							warm_counts);
						return true;
					}
				} else {
...
static void oplus_chg_check_status_full(struct oplus_chg_chip *chip)
{
	...
	if (oplus_warp_get_allow_reading() == false) {
		...
		if (((oplus_warp_get_fastchg_to_normal() == true) ||
			...
			if (fastchg_present_wait_count == FULL_DELAY_COUNTS &&
				...
				if (chip->ffc_support &&
				    chip->ffc_temp_status !=
					    FFC_TEMP_STATUS__HIGH &&
				    chip->ffc_temp_status !=
					    FFC_TEMP_STATUS__LOW) {
					if (chip->vbatt_num == 2 &&
					    chip->dual_ffc == false) {
#ifdef OPLUS_CHG_OP_DEF
...
#ifndef CONFIG_OPLUS_CHARGER_MTK
#ifndef OPLUS_CHG_OP_DEF
	if (chip->wireless_support == true &&
	    oplus_wpc_get_ffc_charging() == true) {
	...
	if ((is_batt_full == 1) ||
		...
		charger_xlog_printk(
			CHG_LOG_CRTI,
			"is_batt_full : %d,  chip->charging_state= %d\n",
		...
		if ((chip->recharge_after_full == true ||
		     chip->vbatt_num == 2) &&
		    (chip->tbatt_when_full <= 450 &&
		     chip->tbatt_when_full >= 0) &&
			...
			if (chip->dod0_counts == DOD0_COUNTS) {
				...
				if (chip->recharge_after_full == true &&
					...
					charger_xlog_printk(
						CHG_LOG_CRTI,
						"recharge after full, dod0_counts = %d\n",
						chip->dod0_counts);
				}
...
static void oplus_chg_kpoc_power_off_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT ||
		...
		if ((chip->chg_ops->check_chrdet_status() == false) &&
			...
			charger_xlog_printk(
				CHG_LOG_CRTI,
				"pmic_thread_kthread]Unplug Charger/USB double check!\n");
			if ((chip->chg_ops->check_chrdet_status() == false) &&
			    (chip->chg_ops->get_charger_volt() < 2500)) {
				if ((oplus_warp_get_fastchg_to_normal() ==
				     false) &&
				    (oplus_warp_get_fastchg_to_warm() ==
				     false) &&
				    (oplus_warp_get_adapter_update_status() !=
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	if (chip->vbatt_num == 1) {
		charger_xlog_printk(
			...
			chip->charger_exist, chip->charger_type,
			chip->charger_volt, chip->prop_status, chip->boot_mode,
			chip->batt_exist, chip->batt_full, chip->chging_on,
			chip->in_rechging, chip->charging_state,
			chip->total_time, chip->temperature, chip->batt_volt,
			chip->batt_volt_min, chip->icharging, chip->ibus,
			...
			oplus_gauge_get_batt_qs(), chip->vbatt_over,
			chip->chging_over_time, chip->vchg_status,
			chip->tbatt_status, chip->stop_voter, chip->notify_code,
			chip->otg_switch, chip->mmi_chg, chip->boot_reason,
			chip->boot_mode, chip->chargerid_volt,
			chip->chargerid_volt_got, chip->shell_temp);
	} else {
		...
		charger_xlog_printk(
			...
			chip->batt_volt_min, chip->icharging, chip->ibus,
			chip->soc, chip->ui_soc, chip->soc_load, chip->batt_rm,
			chip->vbatt_over, chip->chging_over_time,
			chip->vchg_status, chip->tbatt_status, chip->stop_voter,
			chip->notify_code, chip->otg_switch, chip->mmi_chg,
			chip->boot_reason, chip->boot_mode,
			chip->chargerid_volt, chip->chargerid_volt_got,
			chip->shell_temp, chip->otg_online, chip->vph_voltage,
			chip->abnormal_volt_detected, chip->hw_detected);
	}
...
static void oplus_chg_critical_log(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_exist) {
		if (chip->stop_voter == 0 &&
			...
			chg_abnormal_count++;
			if (chg_abnormal_count >=
			    CHARGER_ABNORMAL_DETECT_TIME) {
				chg_abnormal_count =
					CHARGER_ABNORMAL_DETECT_TIME;
			...
			charger_xlog_printk(
				CHG_LOG_CRTI,
				" unable charging, count=%d, charger_abnormal_log=%d\n",
				chg_abnormal_count, charger_abnormal_log);
#ifdef OPLUS_CHG_REG_DUMP_ENABLE
			if (chg_abnormal_count >= 10 && !chip->reg_dump &&
			    is_usb_ocm_available(chip)) {
				chip->reg_dump = true;
				oplus_chg_anon_mod_event(
					chip->usb_ocm,
		...
		charger_xlog_printk(
			CHG_LOG_CRTI,
			" btb_temp_over or fastchg_to_normal, charger_abnormal_log=%d\n",
			charger_abnormal_log);
	} else {
...
static void oplus_chg_ibatt_check_and_set(struct oplus_chg_chip *chip)
{
	...
	if (current_adapt == 0) {
		if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP) {
			...
			if (chip->batt_volt > 4180) {
				current_adapt =
					chip->limits
						.temp_cool_fastchg_current_ma_low;
			} else {
				current_adapt =
					chip->limits
						.temp_cool_fastchg_current_ma_high;
			}
	...
	if (chip->tbatt_status == BATTERY_STATUS__LITTLE_COLD_TEMP) {
		...
		recharge_volt = chip->limits.temp_cool_vfloat_mv -
				chip->limits.recharge_mv;
		if (vbatt_higherthan_4180mv) {
			current_init =
				chip->limits.temp_cool_fastchg_current_ma_low;
		...
		recharge_volt = chip->limits.temp_little_cool_vfloat_mv -
				chip->limits.recharge_mv;
...
static void oplus_chg_pd_config(struct oplus_chg_chip *chip)
{
	...
	if (chip->pd_chging == false &&
	    chip->chg_ops->oplus_chg_get_pd_type() == true &&
		...
		if (ret >= 0) {
			...
			chip->limits.temp_little_cool_fastchg_current_ma =
				chip->limits
					.pd_temp_little_cool_fastchg_current_ma;
			chip->limits.temp_normal_fastchg_current_ma =
				chip->limits.pd_temp_normal_fastchg_current_ma;
			chip->limits.temp_little_cold_fastchg_current_ma_high =
				chip->limits
					.pd_temp_little_cold_fastchg_current_ma_high;
			chip->limits.temp_little_cold_fastchg_current_ma_low =
				chip->limits
					.pd_temp_little_cold_fastchg_current_ma_low;
			chip->limits.temp_cool_fastchg_current_ma_high =
				chip->limits
					.pd_temp_cool_fastchg_current_ma_high;
			chip->limits.temp_cool_fastchg_current_ma_low =
				chip->limits.pd_temp_cool_fastchg_current_ma_low;
			chip->limits.temp_warm_fastchg_current_ma =
				chip->limits.pd_temp_warm_fastchg_current_ma;
...
static void oplus_chg_pdqc_to_normal(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
		if (chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr &&
		    pdqc_9v == true) {
			ret = chip->chg_ops->oplus_chg_pd_setup();
			if (ret >= 0) {
				pdqc_9v = false;
				chip->limits.temp_normal_fastchg_current_ma =
					chip->limits
						.default_temp_normal_fastchg_current_ma;
				chip->limits
					.temp_little_cool_fastchg_current_ma =
					chip->limits
						.default_temp_little_cool_fastchg_current_ma;
				chip->limits
					.temp_little_cool_fastchg_current_ma_high =
					chip->limits
						.default_temp_little_cool_fastchg_current_ma_high;
				chip->limits
					.temp_little_cool_fastchg_current_ma_low =
					chip->limits
						.default_temp_little_cool_fastchg_current_ma_low;
				chip->limits
					.temp_little_cold_fastchg_current_ma_high =
					chip->limits
						.default_temp_little_cold_fastchg_current_ma_high;
				chip->limits
					.temp_little_cold_fastchg_current_ma_low =
					chip->limits
						.default_temp_little_cold_fastchg_current_ma_low;
				chip->limits.temp_cool_fastchg_current_ma_high =
					chip->limits
						.default_temp_cool_fastchg_current_ma_high;
				chip->limits.temp_cool_fastchg_current_ma_low =
					chip->limits
						.default_temp_cool_fastchg_current_ma_low;
				chip->limits.temp_warm_fastchg_current_ma =
					chip->limits
						.default_temp_warm_fastchg_current_ma;
				chip->limits.input_current_charger_ma =
					chip->limits
						.default_input_current_charger_ma;
				oplus_chg_set_charging_current(chip);
				oplus_chg_set_input_current_limit(chip);
			}
		}
...
static void oplus_chg_qc_config(struct oplus_chg_chip *chip)
{
	...
	if (qc_chging == false &&
		...
		if (ret >= 0) {
			...
			chip->limits.temp_little_cool_fastchg_current_ma =
				chip->limits
					.qc_temp_little_cool_fastchg_current_ma;
#endif
			chip->limits.temp_little_cool_fastchg_current_ma_high =
				chip->limits
					.qc_temp_little_cool_fastchg_current_ma;
			...
			chip->limits.temp_little_cold_fastchg_current_ma_high =
				chip->limits
					.qc_temp_little_cold_fastchg_current_ma_high;
			chip->limits.temp_little_cold_fastchg_current_ma_low =
				chip->limits
					.qc_temp_little_cold_fastchg_current_ma_low;
			chip->limits.temp_cool_fastchg_current_ma_high =
				chip->limits
					.qc_temp_cool_fastchg_current_ma_high;
			chip->limits.temp_cool_fastchg_current_ma_low =
				chip->limits.qc_temp_cool_fastchg_current_ma_low;
			chip->limits.temp_warm_fastchg_current_ma =
				chip->limits.qc_temp_warm_fastchg_current_ma;
...
static void oplus_chg_dual_charger_config(struct oplus_chg_chip *chip)
{
	...
	if (chip->slave_charger_enable == false) {
		if (chip->icharging < 0 &&
		    (chip->icharging * -1) > chip->slave_chg_enable_ma) {
			enable_slave_cnt++;
		} else {
			enable_slave_cnt = 0;
		}
...
static void oplus_chg_update_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_chip *chip =
		container_of(dwork, struct oplus_chg_chip, update_work);
#ifdef OPLUS_CHG_OP_DEF
	int shedule_work = 0;
...
bool oplus_chg_wake_update_work(void)
{
	...
	if (!g_charger_chip->update_work.work.func ||
	    !g_charger_chip->update_work.timer.function) {
...
int oplus_chg_show_warp_logo_ornot(void)
{
...
#else /* OPLUS_CHG_OP_DEF */
	if ((oplus_chg_get_wls_charge_type(g_charger_chip) ==
		     OPLUS_CHG_WLS_WARP ||
	     oplus_chg_get_wls_charge_type(g_charger_chip) ==
		     OPLUS_CHG_WLS_SWARP ||
	     oplus_chg_get_wls_charge_type(g_charger_chip) ==
	...
	if (g_charger_chip->chg_ctrl_by_warp) {
		if (oplus_warp_get_fastchg_started() == true &&
		    oplus_warp_get_fast_chg_type() ==
			    CHARGER_SUBTYPE_FASTCHG_WARP) {
			if (g_charger_chip->prop_status !=
				    POWER_SUPPLY_STATUS_FULL &&
			    (g_charger_chip->stop_voter ==
				     CHG_STOP_VOTER__FULL ||
			     g_charger_chip->stop_voter ==
				     CHG_STOP_VOTER_NONE)) {
				return 1;
			} else {
...
int oplus_chg_get_charger_voltage(void)
{
	if (!g_charger_chip) {
		...
		g_charger_chip->charger_volt =
			g_charger_chip->chg_ops->get_charger_volt();
	}
...
int oplus_chg_get_vph_voltage(void)
{
	if (!g_charger_chip) {
		...
		g_charger_chip->vph_voltage =
			g_charger_chip->chg_ops->get_vph_volt();
	}
...
int oplus_chg_get_hw_detect_status(void)
{
	if (!g_charger_chip) {
		...
		g_charger_chip->hw_detected =
			g_charger_chip->chg_ops->get_hw_detect();
	}
	...
	if (chip->smart_charge_user == SMART_CHARGE_USER_USBTEMP) {
		if (chip->smart_charging_screenoff) {
			chip->usbtemp_cool_down =
				oplus_convert_current_to_level(
					chip, USBTEMP_CHARGING_CURRENT_LIMIT);
		} else {
			chip->usbtemp_cool_down = 3;
		}
		if (chip->cool_down) {
			ret = (chip->usbtemp_cool_down < chip->cool_down) ?
				      chip->usbtemp_cool_down :
					    chip->cool_down;
		} else {
		...
		charger_xlog_printk(CHG_LOG_CRTI,
				    "cool_down=%d,usbtemp_cool_down=%d\n",
...
struct current_level SWARP_2_0[] = {
	...
	{ 9, 5000 },   { 10, 5500 },  { 11, 6000 },  { 12, 6300 },
	{ 13, 6500 },  { 14, 7000 },  { 15, 7500 },  { 16, 8000 },
	{ 17, 8500 },  { 18, 9000 },  { 19, 9500 },  { 20, 10000 },
	{ 21, 10500 }, { 22, 11000 }, { 23, 11500 }, { 24, 12000 },
	{ 25, 12500 },
};
...
int oplus_convert_current_to_level(struct oplus_chg_chip *chip, int val)
{
	...
	if (reply_bits == 7) {
		if (chip->vbatt_num == 2) {
			level = find_level_to_current(val, SWARP_2_0,
						      ARRAY_SIZE(SWARP_2_0));
		} else {
			level = find_level_to_current(val, SWARP_1_0,
						      ARRAY_SIZE(SWARP_1_0));
		}
	} else {
		level = val / 1000;
	}
...
int oplus_chg_override_by_shell_temp(int temp)
{
	...
	if ((g_charger_chip->charger_exist) &&
		...
		charger_xlog_printk(CHG_LOG_CRTI,
				    "charging override by shell temperature\n");
		return 1;
	}
	return 0;
}
...
void oplus_smart_charge_by_shell_temp(struct oplus_chg_chip *chip, int val)
{
...
#ifndef CONFIG_OPLUS_CHG_OOS
#ifdef OPLUS_CHG_OP_DEF
	union oplus_chg_mod_propval opval = {
		0,
	};
#endif
#endif
	...
	if (chip->shell_themal) {
		rc = thermal_zone_get_temp(chip->shell_themal,
					   &chip->shell_temp);
		if (rc) {
	...
	charger_xlog_printk(
		CHG_LOG_CRTI,
		"val->intval = [%04x], shell_temp = [%04x], set shell_temp_down = [%d].\n",
	...
	if (!val) {
		if (chip->cool_down != 0) {
			...
			chip->limits.input_current_warp_ma_normal =
				chip->limits
					.default_input_current_warp_ma_normal;
			chip->limits.pd_input_current_charger_ma =
				chip->limits.default_pd_input_current_charger_ma;
			chip->limits.qc_input_current_charger_ma =
				chip->limits.default_qc_input_current_charger_ma;
...
#else
	rc = oplus_chg_mod_get_property(chip->usb_ocm,
					OPLUS_CHG_PROP_FAST_CHG_TYPE, &opval);
	if (rc < 0) {
		pr_err("Failed to read charging subtype\n");
		return;
	}
...
static const int cool_down_current_limit_normal[6] = { 1500, 1500, 1500,
						       1500, 2000, 2000 };
static const int cool_down_current_limit_warp[6] = { 1000, 1500, 2000,
						     2500, 3000, 3500 };
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	if (val > 6) {
		...
		chip->limits.qc_input_current_charger_ma =
			cool_down_current_limit_normal[6 - 1];
		chip->limits.input_current_warp_ma_high =
			cool_down_current_limit_warp[6 - 1];
		chip->limits.input_current_warp_ma_warm =
			cool_down_current_limit_warp[6 - 1];
		chip->limits.input_current_warp_ma_normal =
			cool_down_current_limit_warp[6 - 1];
		...
		chip->limits.qc_input_current_charger_ma =
			cool_down_current_limit_normal[val - 1];
		chip->limits.input_current_warp_ma_high =
			cool_down_current_limit_warp[val - 1];
		chip->limits.input_current_warp_ma_warm =
			cool_down_current_limit_warp[val - 1];
		chip->limits.input_current_warp_ma_normal =
			cool_down_current_limit_warp[val - 1];
...
int oplus_tbatt_power_off_task_init(struct oplus_chg_chip *chip)
{
	...
	chip->tbatt_pwroff_task = kthread_create(oplus_tbatt_power_off_kthread,
						 chip, "tbatt_pwroff");
	...
	if (!g_charger_chip) {
		...
		if (g_charger_chip->chg_ops->input_current_write_without_aicl &&
			...
			g_charger_chip->chg_ops
				->input_current_write_without_aicl(current_ma);
...
void oplus_chg_config_charger_vsys_threshold(int val)
{
	if (!g_charger_chip) {
		...
		if (g_charger_chip->chg_ops->set_charger_vsys_threshold &&
		    oplus_warp_get_allow_reading() == true) {
			g_charger_chip->chg_ops->set_charger_vsys_threshold(
				val);
			chg_err("set val[%d]\n", val);
		}
	}
...
void oplus_chg_enable_burst_mode(bool enable)
{
	if (!g_charger_chip) {
		...
		if (g_charger_chip->chg_ops->enable_burst_mode &&
		    oplus_warp_get_allow_reading() == true) {
...
void oplus_chg_update_float_voltage_by_fastchg(bool fastchg_en)
{
	...
	if (fastchg_en) {
		g_charger_chip->chg_ops->float_voltage_write(
			flv * g_charger_chip->vbatt_num);
