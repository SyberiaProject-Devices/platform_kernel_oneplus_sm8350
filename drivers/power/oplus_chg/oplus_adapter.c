...
static void oplus_adpater_awake_init(struct oplus_adapter_chip *chip)
{
	...
	wake_lock_init(&chip->adapter_wake_lock, WAKE_LOCK_SUSPEND,
		       "adpater_wake_lock");
#else
...
static void adapter_update_work_func(struct work_struct *work)
{
	...
	struct oplus_adapter_chip *chip = container_of(
		dwork, struct oplus_adapter_chip, adapter_update_work);
	...
	if (update_result) {
		oplus_warp_set_adapter_update_real_status(
			ADAPTER_FW_UPDATE_SUCCESS);
	} else {
		oplus_warp_set_adapter_update_real_status(
			ADAPTER_FW_UPDATE_FAIL);
		oplus_warp_set_adapter_update_report_status(
			ADAPTER_FW_UPDATE_FAIL);
	}
	msleep(20);
