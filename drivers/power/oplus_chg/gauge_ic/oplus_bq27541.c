...
module_param_named(debug_temp_mask, __debug_temp_mask, int, 0600);
...
static int bq28z610_get_balancing_config(void)
{
	...
	if (gauge_ic->batt_bq28z610) {
		if (oplus_warp_get_allow_reading() == true) {
			...
			tmp = ((data[3] << 24 | data[2] << 16 | data[1] << 8 |
				data[0]) &
...
#define TEMP_LT_16C 1
#define TEMP_LT_39C 2
#define TEMP_HT_39C 3
...
static struct oplus_gauge_operations bq27541_gauge_ops = {
	...
	.get_prev_batt_remaining_capacity =
		bq27541_get_prev_batt_remaining_capacity,
	...
	.get_prev_battery_mvolts_2cell_max =
		bq27541_get_prev_battery_mvolts_2cell_max,
	.get_prev_battery_mvolts_2cell_min =
		bq27541_get_prev_battery_mvolts_2cell_min,
...
static void bq27541_hw_config(struct chip_bq27541 *chip)
{
	...
	dev_err(chip->dev,
		"DEVICE_TYPE is 0x%02X, FIRMWARE_VERSION is 0x%02X\n",
		device_type, fw_ver);
}
...
static void bq27541_parse_dt(struct chip_bq27541 *chip)
{
	...
	chip->battery_full_param =
		of_property_read_bool(node, "qcom,battery-full-param");
...
static int bq28z610_write_flash_busy_wait_i2c_err(struct chip_bq27541 *chip)
{
	...
	msleep(100);
	bq27541_i2c_txsubcmd(BQ28Z610_REG_CNTL1, 0x4603);
	msleep(100);
...
int bq28z610_write_soc_smooth_parameter(struct chip_bq27541 *chip)
{
	...
	u8 CNTL1_write2[BQ28Z610_REG_CNTL1_SIZE] = { 0x08, 0x47, 0x96, 0x00 };
	...
	bq27541_i2c_txsubcmd(BQ28Z610_REG_CNTL2, 0x061A);
...
static int gauge_reg_dump(void)
{
	...
	if (oplus_warp_get_allow_reading() == true) {
		...
		for (; i < sizeof(dump_reg) / sizeof(int) && sum < len - 16;
		     i++) {
			ret = bq27541_read_i2c(dump_reg[i], &val);
			if (ret) {
				dev_err(gauge_ic->dev,
					"[OPLUS_CHG]:error reading regdump, ret:%d\n",
					ret);
				return -1;
			}
			l = sprintf(pos, "/ %d ", val);
...
static void bq27541_reset(struct i2c_client *client)
{
	...
	if (bq27541_gauge_ops.get_battery_mvolts() <= 3300 &&
	    bq27541_gauge_ops.get_battery_mvolts() > 2500 && ui_soc == 0 &&
		...
		chg_debug(
			...
			bq27541_gauge_ops.get_battery_mvolts(), ui_soc,
			bq27541_gauge_ops.get_battery_temperature());
		if (gauge_ic->device_type == DEVICE_BQ27541 ||
		    gauge_ic->device_type == DEVICE_ZY0602) {
...
*/
	...
	if (rc == BATT_FULL_ERROR && rerun_num > 0)
		goto rerun;
	chip = devm_kzalloc(&client->dev, sizeof(struct oplus_gauge_chip),
			    GFP_KERNEL);
	if (!chip) {
