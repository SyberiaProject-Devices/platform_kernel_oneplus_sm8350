...
int oplus_chg_wls_rx_set_dcdc_enable(struct oplus_wls_chg_rx *wls_rx)
{
	struct oplus_chg_ic_dev *rx_ic;
	struct oplus_chg_ic_rx_ops *rx_ic_ops;
	int rc;
	...
	rx_ic_ops = rx_ic->dev_ops;
	rc = rx_ic_ops->rx_set_dcdc_enable(rx_ic);
...
int oplus_chg_wls_rx_upgrade_firmware_by_img(struct oplus_wls_chg_rx *wls_rx)
{
	...
	rc = rx_ic_ops->rx_get_fw_version_by_chip(rx_ic, &old_version);
	if (rc < 0) {
	...
	if (new_version != old_version) {
		...
		snprintf(rx_ic->fw_id, 15, "0x%08x", new_version);
	} else
		snprintf(rx_ic->fw_id, 15, "0x%08x", old_version);
#ifdef CONFIG_OPLUS_CHG_OOS
					     ...
					     unsigned char *buf, int len)
{
	...
	__pm_stay_awake(wls_rx->update_fw_wake_lock);
	wls_rx->wls_dev->wls_status.fw_upgrading = true;
	rc = rx_ic_ops->rx_get_fw_version_by_buf(buf, len, &fw_version);
	if (rc < 0) {
		pr_err("can't get fw version by buf, rc=%d\n", rc);
		goto out;
	}
	pr_info("rx new_version=0x%08x\n", fw_version);
...
int oplus_chg_wls_rx_smt_test(struct oplus_wls_chg_rx *wls_rx)
{
	...
	rc = rx_ic_ops->rx_get_fw_version_by_chip(rx_ic, &version);
	if (rc < 0) {
		pr_err("can't get fw version by img, rc=%d\n", rc);
