...
struct sched_avg_stats stats[NR_CPUS];
...
struct sched_avg_stats *sched_get_nr_running_avg(void)
{
	...
	for_each_sched_cluster(cluster) {
		...
		for_each_cpu(cpu, &cluster->cpus) {
			 ...
			 */
			sum_util_pct +=
				(rq->wrq.util * 100) / arch_scale_cpu_capacity(cpu);
		}
...
static inline void update_busy_hyst_end_time(int cpu, int inc,
 ...
 * @inc: enqueue/dequeue/misfit happening on this CPU.
 * @return: N/A
 *
 * Update average with latest nr_running value for CPU
 */
void sched_update_nr_prod(int cpu, long delta, int inc)
{
	u64 diff;
	u64 curr_time;
